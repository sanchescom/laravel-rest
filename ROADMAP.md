...
- **Eager loading.** `Post::with('comments')->get()` — collect parent keys,
  batch the relation via a single `whereIn` query (or concurrent `getMany`),
  distribute onto instances. Eliminates the HTTP N+1 that lazy relations
  currently produce.
...
- **HTTP-level caching** — ETag / Cache-Control conditional GET as a Guzzle
  middleware, orthogonal to the application-level cache (design sketched in
  `docs/superpowers/specs/2026-09-05-caching-research.md`).
